...
deploy_components() {
  ...
  if [ "$PROJECT" == "mail-pipeline" ]; then
    ...
    COMPONENTS=("roles" "prometheus" "recommender" "persistence" "image" "entrypoint" "auth")
  else
  ...
  for COMPONENT in "${COMPONENTS[@]}"; do
    if [ "$COMPONENT" == "prometheus" ]; then
      ...
      if [ -d "$PROJECT_DIR/$COMPONENT" ]; then
        ...
        echo -e "${YELLOW}Skipping $COMPONENT: directory not found for global mode.${NC}"
      fi
      ...
      if [ -f "$FILE_PATH" ]; then
        ...
        echo -e "${YELLOW}Skipping $COMPONENT: $COMPONENT.yaml not found for local mode.${NC}"
      fi
...
undeploy_components() {
  ...
  COMPONENTS=("virus_scanner" "message_analyzer" "image_analyzer" "text_analyzer" "link_analyzer" "attachment_manager" "header_analyzer" "parser" "entrypoint" "gs-algorithm" "redis" "prometheus" "roles")
